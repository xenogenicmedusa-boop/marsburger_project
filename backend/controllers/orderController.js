...
const baseSelect = `SELECT o.id,o.order_number AS orderNumber,o.customer_name AS customerName,o.customer_count AS customerCount,o.memo,o.meals,o.pay_type AS payType,o.pickup_type AS pickupType,o.selected_count AS selectedCount,o.total_amount AS totalPrice,o.status,o.created_at AS createdAt,u.username FROM orders o LEFT JOIN users u ON u.id=o.user_id`;

exports.createOrder = async (req, res, next) => {
  const { customerName, customerCount = 1, memo = '', meals, payType, pickupType, totalPrice } = req.body;
  if (!customerName?.trim() || !Array.isArray(meals) || !meals.length || !payType || !pickupType || !Number.isFinite(Number(totalPrice)) || Number(totalPrice) < 0) return res.status(400).json({ message: '訂單資料不完整。' });
  try {
    const orderNumber = `MARS-${Date.now()}-${Math.floor(Math.random() * 900 + 100)}`;
    const [r] = await pool.query(`INSERT INTO orders (user_id,order_number,customer_name,customer_count,memo,meals,pay_type,pickup_type,selected_count,total_amount) VALUES (?,?,?,?,?,?,?,?,?,?)`, [req.user?.userId || null, orderNumber, customerName.trim(), customerCount, memo, JSON.stringify(meals), payType, pickupType, meals.reduce((n, meal) => n + Number(meal.quantity || 1), 0), Number(totalPrice)]);
