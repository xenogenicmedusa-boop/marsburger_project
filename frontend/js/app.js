...
function renderMenu() { $('#menu').innerHTML = menu.map((item, index) => `<div class="col-md-4 mb-3"><div class="card food-card h-100 overflow-hidden"><img src="images/${item.name}.jpg" alt="${item.label}" class="card-img-top" loading="lazy" decoding="async" width="640" height="160" style="height:160px;object-fit:cover"><div class="card-body d-flex flex-column"><h5>${item.label}</h5><p class="price mt-auto">${money(item.price)}</p><button type="button" class="btn btn-outline-danger" onclick="addCart(${index})">加入購物車</button></div></div></div>`).join(''); }
